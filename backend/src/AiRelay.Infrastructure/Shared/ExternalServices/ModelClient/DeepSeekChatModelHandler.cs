...
using AiRelay.Infrastructure.Shared.ExternalServices.ModelClient.Processor.DeepSeek;
...
/// <summary>
/// DeepSeek 官方 API：一把 Key 同时转发 Chat Completions、Responses、Anthropic Messages。
/// </summary>
public class DeepSeekChatModelHandler(
    ChatModelConnectionOptions options,
    IModelProvider modelProvider,
    IHttpClientFactory httpClientFactory,
    ILogger<DeepSeekChatModelHandler> logger)
    : BaseChatModelHandler(options, httpClientFactory, logger)
{
    ...
    protected override IReadOnlyList<IRequestProcessor> GetRequestProcessors(DownRequestContext down, int degradationLevel)
    {
        return
        [
            new DeepSeekUrlRequestProcessor(Options),
            new DeepSeekHeaderRequestProcessor(Options),
    ...
    {
        IResponseProcessor sse = DeepSeekUrlRequestProcessor.IsAnthropicRoute(up.RelativePath)
    ...
    public override async Task<IReadOnlyList<ModelOption>?> GetModelsAsync(CancellationToken ct = default)
    {
        ...
        {
            Logger.LogWarning("DeepSeek 上游模型拉取失败: {StatusCode}, URL={Url}", response.StatusCode, requestUrl);
            return null;
        }
        ...
        {
            Logger.LogWarning(
                "DeepSeek 上游返回 0 模型: URL={Url}, JsonLength={Len}",
                requestUrl, json.Length);
            return null;
        }

        Logger.LogInformation("DeepSeek 上游拉取成功: {Count} 个模型", models.Count);
        return models;
    }
